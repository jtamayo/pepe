...
/**
 * Default class for instrumenting most methods. It reads the taint from method
 * parameters; propagates it through arithmetic operations, field accesses and
 * method invocations; and it returns the taint through the return value.
 * 
 * @author jtamayo
 */
public class PlainMethodVisitor implements MethodVisitor, Opcodes {
		...
		public PlainMethodVisitor(MethodVisitor output, MethodNode mn, Frame[] frames, ClassNode cn) {
			this.output = output;
			this.mn = mn;
			this.frames = frames;
			this.cn = cn;
		...
		public void visitCode() {
			...
			// TODO: this might be a mistake, because if the method were to be <init>
			// I wouldn't be invoking the super constructor as the first thing in the method
			// Perhaps it can be fixed by inheriting from AdviceAdapter, and instead of emitting loadCurrentThread
			// here we'd emit it onMethodEnter
		...
		public void visitFieldInsn(int opcode, String owner, String name, String desc) {
			...
			case GETSTATIC:
				...
				output.visitFieldInsn(opcode, owner, name, desc);
				if (InstrumentationPolicy.isFieldInstrumentable(owner, name, true)) {
...
//			output.visitFrame(type, nLocal, local, nStack, stack);
		...
		public void visitMethodInsn(int opcode, String owner, String name, String desc) {
			...
			case INVOKESPECIAL:
				...
				output.visitMethodInsn(opcode, owner, name, desc);
				if (name.equals("<init>") && InstrumentationPolicy.isPrimitiveWrapperOrString(owner)) {
					// For the constructor, the reference to the newly created object is not returned from
					// the method, but it is instead on the stack. Taint that reference instead.
					// BRITTLE: <init> is not only called when creating a new object, but also
					// by the <init> method itself when invoking the super constructor, or when invoking
					// another constructor. In those cases it's not safe to assume there is a reference
					// to the newly created object in the stack. However, the wrapper/string constructors 
					// only call the object constructor, so we're safe when assuming that if the receiver
					// of the init method is a wrapper/string, there is a copy of the reference in the stack.
					loadReturnValueTaint();
					output.visitVarInsn(LSTORE, shadowStackIndex - SHADOW_FIELD_SIZE * (argSize + 2)); // One for the "this" parameter plus one for the copy of the object ref
				} else if (!Type.getReturnType(desc).equals(Type.VOID_TYPE)) {
		...
		/**
		 * Clears java.lang.Thread return value, to avoid reading stale values
		 * in case the invoked method is not instrumented
		 */
		private void emitClearReturnValue() {
			Label l1 = new Label();
			output.visitLabel(l1);
			output.visitVarInsn(ALOAD, currentThreadIndex);
			Label l2 = new Label();
