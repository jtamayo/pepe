...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    static final class Segment<K,V> implements Serializable {
         ...
         */
        synchronized V readValueUnderLock(HashEntry<K,V> e) {
//            lock();
            try {
                return e.value;
            } finally {
...
//                unlock();
        ...
        synchronized V replace(K key, int hash, V newValue) {
//            lock();
            try {
                HashEntry<K,V> e = getFirst(hash);
                while (e != null && (e.hash != hash || !key.equals(e.key)))
...
//                unlock();
        ...
        synchronized V put(K key, int hash, V value, boolean onlyIfAbsent) {
//            lock();
            try {
                int c = count;
...
//                unlock();
         ...
         */
        synchronized V remove(Object key, int hash, Object value) {
//            lock();
            try {
                int c = count - 1;
                HashEntry[] tab = table;
...
//                    unlock();
    ...
    public int size() {
        ...
        if (check != sum) { // Resort to locking all segments
            sum = 0;
            for (int i = 0; i < segments.length; ++i) 
            	sun.misc.Unsafe.getUnsafe().monitorEnter(segments[i]);
//                segments[i].lock();
            for (int i = 0; i < segments.length; ++i) 
                sum += segments[i].count;
            for (int i = 0; i < segments.length; ++i)
            	sun.misc.Unsafe.getUnsafe().monitorExit(segments[i]);
//                segments[i].unlock();
        }
...
//                segments[i].unlock();
    ...
    private void writeObject(java.io.ObjectOutputStream s) throws IOException  {
        ...
        for (int k = 0; k < segments.length; ++k) {
            ...
            Unsafe.getUnsafe().monitorEnter(seg);
//            seg.lock();
            try {
            	...
            	Unsafe.getUnsafe().monitorExit(seg);
//                seg.unlock();
            }
        }
        s.writeObject(null);
